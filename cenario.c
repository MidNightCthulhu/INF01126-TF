...
int cenario_salva(Cenario* cena, const char* nomeArq){
    ...
    for(int i = 0; i < cena->nEntes; i++){
        ...
        if(fprintf(arquivo, "%d,%d,%d,%d,%d,%c\n", enteAux->chave, enteAux->noMapa, enteAux->tregua,
            enteAux->x, enteAux->y, enteAux->tipo) < 12){                                               //caso a escrita falhe
            fclose(arquivo);                                                                            //desaloca a struct FILE
        return 0;                                                                                       //retorna o valor 0(zero), significando que nao salvou a struct Cenario no arquivo
        }
    }
...
Cenario* cenario_carrega(const char* nomeArq){
    ...
    for(int i = 0; i < cena->nEntes; i++){
        ...
        if(fscanf(arquivo, "%d,%d,%d,%d,%d,%c\n", &enteAux->chave, &enteAux->noMapa, &enteAux->tregua,
            &enteAux->x, &enteAux->y, &enteAux->tipo) != 6){                                            //caso a leitura falhe
            free(cena->entes);                                                                          //desaloca o vetor de ponteiros para Entidade
            mapa_destroi(cena->mapa);                                                                   //desaloca a struct Mapa
            free(cena);                                                                                 //desaloca a struct Cenario
            fclose(arquivo);                                                                            //desaloca a struct FILE
...
int* cenario_F_ataca(Cenario* cena, int* tamF){
    ...
    int* caminhoF = dijkstra_reconstroi_caminho(f, (cena->entes[cena->chaveP]->y * cena->mapa->col) + cena->entes[cena->chaveP]->x, &tamF);//obtem o vetor com a sequencia dos vertices entre o vertice origem do fantasma e o vertice destino do Pac-Man
    dijkstra_destroi(f);
    return caminhoF;
}

int* cenario_G_ataca(Cenario* cena, int* tamG){
    Dijkstra* g = dijkstra(cena->grafo, (cena->entes[cena->chaveG]->y * cena->mapa->col) + cena->entes[cena->chaveG]->x);//alocamos uma struct Dijkstra baseada no Digrafo da struct Cenario e com origem no vertice da Entidade fantasma F
    int* caminhoG = dijkstra_reconstroi_caminho(g, (cena->entes[cena->chaveP]->y * cena->mapa->col) + cena->entes[cena->chaveP]->x, &tamG);//obtem o vetor com a sequencia dos vertices entre o vertice origem do fantasma e o vertice destino do Pac-Man
    dijkstra_destroi(g);
    return caminhoG;
}

int* cenario_H_ataca(Cenario* cena, int* tamH){
    Dijkstra* h = dijkstra(cena->grafo, (cena->entes[cena->chaveH]->y * cena->mapa->col) + cena->entes[cena->chaveH]->x);//alocamos uma struct Dijkstra baseada no Digrafo da struct Cenario e com origem no vertice da Entidade fantasma F
    int* caminhoH = dijkstra_reconstroi_caminho(h, (cena->entes[cena->chaveP]->y * cena->mapa->col) + cena->entes[cena->chaveP]->x, &tamH);//obtem o vetor com a sequencia dos vertices entre o vertice origem do fantasma e o vertice destino do Pac-Man
    dijkstra_destroi(h);
    return caminhoH;
}

int* cenario_J_ataca(Cenario* cena, int* tamJ){
    Dijkstra* j = dijkstra(cena->grafo, (cena->entes[cena->chaveJ]->y * cena->mapa->col) + cena->entes[cena->chaveJ]->x);//alocamos uma struct Dijkstra baseada no Digrafo da struct Cenario e com origem no vertice da Entidade fantasma F
    int* caminhoJ = dijkstra_reconstroi_caminho(j, (cena->entes[cena->chaveP]->y * cena->mapa->col) + cena->entes[cena->chaveP]->x, &tamJ);//obtem o vetor com a sequencia dos vertices entre o vertice origem do fantasma e o vertice destino do Pac-Man
    dijkstra_destroi(j);
    return caminhoJ;
}
